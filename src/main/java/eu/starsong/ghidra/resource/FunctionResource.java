...
public class FunctionResource implements Resource {
    ...
    public void register(Javalin app, java.util.function.Function<Context, GhidraContext> contextFactory) {
        ...
        app.get("/functions/{address}/cfg", ctx -> cfg(contextFactory.apply(ctx)));
        app.get("/functions/{address}/pcode", ctx -> pcode(contextFactory.apply(ctx)));
        ...
        app.get("/functions/by-name/{name}/cfg", ctx -> cfgByName(contextFactory.apply(ctx)));
        app.get("/functions/by-name/{name}/pcode", ctx -> pcodeByName(contextFactory.apply(ctx)));
    }
    ...
    private void cfg(GhidraContext ctx) {
        var program = ctx.requireProgram();
        String address = ctx.pathParam("address");
        Function fn = functionService.requireFunctionByAddress(program, address);
        var cfg = functionService.getCfg(program, fn);
        ctx.json(Response.ok(ctx.ctx(), ctx.port(), cfg)
            .self("/functions/{}/cfg", address)
            .link("function", "/functions/{}", address)
            .build());
    }

    private void cfgByName(GhidraContext ctx) {
        var program = ctx.requireProgram();
        String name = ctx.pathParam("name");
        Function fn = functionService.requireFunctionByName(program, name);
        var cfg = functionService.getCfg(program, fn);
        ctx.json(Response.ok(ctx.ctx(), ctx.port(), cfg)
            .self("/functions/by-name/{}/cfg", name)
            .link("function", "/functions/by-name/{}", name)
            .build());
    }

    private void pcode(GhidraContext ctx) {
        var program = ctx.requireProgram();
        String address = ctx.pathParam("address");
        int timeout = ctx.queryParamAsInt("timeout", 60);
        Function fn = functionService.requireFunctionByAddress(program, address);
        var pcode = functionService.getPcode(program, fn, timeout);
        ctx.json(Response.ok(ctx.ctx(), ctx.port(), pcode)
            .self("/functions/{}/pcode", address)
            .link("function", "/functions/{}", address)
            .build());
    }

    private void pcodeByName(GhidraContext ctx) {
        var program = ctx.requireProgram();
        String name = ctx.pathParam("name");
        int timeout = ctx.queryParamAsInt("timeout", 60);
        Function fn = functionService.requireFunctionByName(program, name);
        var pcode = functionService.getPcode(program, fn, timeout);
        ctx.json(Response.ok(ctx.ctx(), ctx.port(), pcode)
            .self("/functions/by-name/{}/pcode", name)
            .link("function", "/functions/by-name/{}", name)
            .build());
    }
    ...
    private Response functionResponse(GhidraContext ctx, FunctionDto fn, String address) {
        return Response.ok(ctx.ctx(), ctx.port(), fn)
            ...
            .link("variables", "/functions/{}/variables", address)
            .link("cfg", "/functions/{}/cfg", address)
            .link("pcode", "/functions/{}/pcode", address)
